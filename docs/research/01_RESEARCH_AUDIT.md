...
| Monte Carlo strategy + regret | GREEN | Built. 50,000 races (10,000 × 5 strategies) in 33 ms. |
| Cross-domain transfer | GREEN | Built on NASA C-MAPSS with real ground truth. RUL RMSE 22.7 cycles over **all 100** FD001 test engines — the set published figures use, so like-for-like. |
...
- To beat purpose-built prognostics models on C-MAPSS. Published RMSE there is
  12–20 cycles; ours is 22.7, on the same 100 engines they are quoted over.
